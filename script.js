...
function checkAnswer(currentLevel) {
    ...
    if (gamePattern[currentLevel] === userClickedPattern[currentLevel]) {
        ...
        // Add 'game-over' class to the body of the website when the user gets one of the answers wrong and then remove it after 200 milliseconds.
        $('body').addClass('game-over');
        setTimeout(function () {
            $('body').removeClass('game-over')
        }, 200)

        // Change the h1 title accordingly
...
function startOver() {
    ...
    // Reset level, gamePattern and started so the game starts over.
    level = 0;
    gamePattern = [];
    gameStarted = false;
}